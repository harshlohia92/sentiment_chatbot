...
def predict(model, text, max_length=128):
    ...
    with torch.no_grad():
        ...
        logits = output.logits  # shape: [batch, num_labels]

        # Handle 2-class or 1-class output properly
        if logits.shape[-1] == 2:
        ...
        pred_value = prediction[0].item() if prediction.numel() > 0 else 0
        return 'positive' if pred_value == 1 else 'negative'
